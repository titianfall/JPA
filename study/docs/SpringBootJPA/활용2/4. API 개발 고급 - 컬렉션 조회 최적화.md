...
컬렉션 조회는 JPQL 문자열을 직접 쓰는 코드가 많아서 유독 많이 틀렸다.

### ① `createQuery`를 `createNamedQuery`로 씀

```java
em.createNamedQuery("select new ...OrderQueryDto(...) from Order o ...", OrderQueryDto.class) // ❌
```

```
org.hibernate.query.UnknownNamedQueryException:
No query named 'select new jpabook.jpashop.repository.order.query.OrderQueryDto(...)'
```

`createNamedQuery`는 `@NamedQuery`로 **등록해 둔 이름**을 받는다. JPQL 문자열을 넘기니 그 긴 문자열을 쿼리 "이름"으로 찾다가 실패한 것. → `createQuery`

### ② 필드명 `o.status`

강의는 `o.status`지만 **내 `Order`의 필드명은 `orderStatus`** 다. JPQL은 테이블 컬럼이 아니라 **엔티티 필드명**을 쓴다.

### ③ `select` 뒤에 `new` 누락

```java
"select jpabook.jpashop...OrderItemQueryDto(...)"      // ❌ 함수 호출로 해석하려다 실패
"select new jpabook.jpashop...OrderItemQueryDto(...)"  // ✅
```

### ④ 엔티티 이름 오타 `Orderitem`

```
Could not resolve root entity 'Orderitem'
```

JPQL의 엔티티 이름은 **대소문자를 구분**한다. 테이블명(`order_item`)이 아니라 **클래스명(`OrderItem`)** 이다.

### ⑤ 생성자 파라미터 순서가 역순

```java
// ❌ JPQL은 (Long, String, LocalDateTime, OrderStatus, Address) 순으로 넘기는데
public OrderQueryDto(Address address, String orderStatus, LocalDateTime orderDate, String name, Long orderId)
```

`new` 생성자 표현식은 **순서와 타입이 정확히 일치**해야 생성자를 찾는다. 게다가 `orderStatus`를 `String`으로 선언해 둬서 `OrderStatus` 열거형과도 안 맞았다.

### ⑥ 필드명이 값과 반대 — `itemId`에 주문 ID

```java
// ❌ 생성자로 들어오는 값은 oi.order.id (주문 ID)인데 필드명은 ItemId
private Long ItemId;
```
...
`groupingBy(dto -> dto.getItemId())`가 **우연히 동작은 했다**(값 자체는 주문 ID였으니까). 하지만 응답 JSON에 `"itemId": 1`처럼 주문 ID가 상품 ID인 척 나갔다.

💡 강의에서는 이 필드에 `@JsonIgnore`를 붙여 응답에서 숨긴다. 그룹핑 key로만 쓰는 값이라 클라이언트에게는 의미가 없기 때문이다.
...
| 14 | JPQL은 **엔티티 필드명·클래스명**을 쓰고 대소문자를 구분한다 | `orderStatus`, `OrderItem` |
