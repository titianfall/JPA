...
> `4. 쿼리 메소드 기능.pdf` 강의자료 기준으로 **미리 정리**해 둔 문서다.
> Spring Boot 4.1.0, spring-data-jpa 4.1.0, Hibernate 7.4.1 기준.
>
> 🚧 **아직 코드로 옮기지 않은 절은 `🚧 미구현` 으로 표시했다.**
> 현재 내 코드에 있는 것은 3장에서 선언한 `MemberRepository.findByUsername` 하나뿐이다.
...
## 3. `@Query` — 리포지토리 메소드에 쿼리 직접 정의 🚧 미구현
...
## 9. `@EntityGraph` — 페치 조인의 간편 버전 🚧 미구현
...
**순수 JPA (`MemberJpaRepository`)**
- [ ] `findByUsernameAndAgeGreaterThan`
- [ ] `findByPage`, `totalCount` (페이징)
- [ ] `bulkAgePlus` (벌크 수정)

**스프링 데이터 JPA (`MemberRepository`)**
- [ ] `findByUsernameAndAgeGreaterThan` (메서드 이름)
- [ ] `@NamedQuery` + `findByUsername` (NamedQuery — 맛만 보고 넘어간다)
- [ ] `@Query` 로 `findUser`
- [ ] `findUsernameList` (값 조회), `findMemberDto` (DTO 조회) + `MemberDto` 클래스
- [ ] `findByNames` (컬렉션 in 절)
- [ ] 반환 타입 3종 비교 테스트
- [ ] `findByAge(int, Pageable)` → `Page` / `Slice` / `List` / count 분리
- [ ] `@Modifying` + `bulkAgePlus`
- [ ] `@EntityGraph` 3가지 형태 + `findMemberFetchJoin`
- [ ] `@QueryHints` readOnly, `@Lock`
