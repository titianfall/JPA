...
⚠️ **Initializr가 만들어 준 패키지명은 고쳐야 한다.**
artifactId 가 `data-jpa` 라서 Initializr는 하이픈을 `_` 로 바꾼 **`study.data_jpa`** 를 생성해 준다.
하지만 **자바 패키지 명명 관례상 언더스코어는 쓰지 않는다** — 전부 소문자로 이어 붙이는 게 표준이다.
그래서 **`study.datajpa`** 로 바꿨다. 강의 패키지명과도 일치하므로 코드를 그대로 참고할 수 있다.

| | 패키지 |
|---|---|
| Initializr 생성값 | `study.data_jpa` ❌ (관례 위반) |
| **내 프로젝트 / 강의** | **`study.datajpa`** ✅ |

💡 artifactId(`data-jpa`)와 패키지명(`datajpa`)이 달라도 상관없다. Gradle 산출물 이름과 자바 패키지는 별개다.
...
@SpringBootTest
@Transactional
// @Rollback(false) // 3장에서 제거 — 켜두면 데이터가 쌓여 개수 검증이 깨진다
...
flowchart LR
    ...
    C --> D[EntityManager]
    D --> E[(H2)]
    style A fill:#e8f0fe,color:#1a1a1a,stroke:#5a6270,stroke-width:1px
    style B fill:#fff3cd,color:#1a1a1a,stroke:#5a6270,stroke-width:1px
```
...
| 항목 | 결론 |
|------|------|
| 프로젝트 | Gradle-Groovy, group `study`, artifact `data-jpa` → 패키지는 `study.datajpa` (Initializr 기본값 `study.data_jpa` 에서 수정) |
